...
function _getLang(path){if(!path)return'js';const base=path.split('/').pop()||'';if(['Dockerfile'].includes(base))return'dockerfile';if(['Makefile','Procfile','LICENSE','README','CHANGELOG'].some(n=>base.startsWith(n)))return'plain';const i=path.lastIndexOf('.');if(i<0)return'plain';return _extLang[path.substring(i).toLowerCase()]||'js'}
