...
function postDiagnostic(event, extra = {}) {
  ...
  let payload = createXRThreeDiagnosticPayload({
    ...
    surface: {
      ...
      panelContentKind: 'baseline-live-dom',
    },
    secureContext: window.isSecureContext,
...
function getHtmlCanvasDiagnosticsPayload() {
  ...
  let support = livePanelPrepareSummary?.support || htmlCanvasRenderer.getSupport();
  return {
    ...htmlCanvas,
    threeTexture: createXRThreeTextureCapabilitySummary(THREE, support),
    responseHeader: {
...
function getLaunchGateSummary(options = {}) {
  return createWebXRLaunchGateSummary(support, {
    ...
    selectedMode: mode,
    launch: launchRecommendation,
    texture: options.texture || getTextureReadiness(),
    userActivation: window.navigator?.userActivation || null,
...
function renderStatus(extra = {}) {
  ...
  setRows([
    ...
    ['Surface', 'harness:xr-three-panels-baseline'],
    ['Production', 'no'],
    ...
    ['Three HTMLTexture', htmlCanvasDiagnostics.threeTexture?.htmlTextureAvailable ? `available:${htmlCanvasDiagnostics.threeTexture.threeRevision || 'unknown'}` : htmlCanvasDiagnostics.threeTexture?.reason || 'missing'],
    ['Three texture capability', htmlCanvasDiagnostics.threeTexture?.ready ? 'ready' : htmlCanvasDiagnostics.threeTexture?.reason || '-'],
    ...
    ['Strict rendered panels', `${adapterState.renderedPanelCount || 0}/${adapterState.panelCount || 0}`],
    ['Strict diagnostic panels', adapterState.diagnosticPanelCount ? `${adapterState.diagnosticPanelCount}: ${adapterState.diagnosticPanelIds.join(', ')}` : '-'],
    ...
    ['Session base layer', telemetry.renderState?.baseLayer ? (telemetry.renderState.baseLayer.present ? `${telemetry.renderState.baseLayer.framebufferWidth || '-'}x${telemetry.renderState.baseLayer.framebufferHeight || '-'}` : 'missing') : '-'],
    ['Session viewports', telemetry.viewports ? `${telemetry.viewports.viewCount}: ${telemetry.viewports.views?.map((view) => `${view.eye || '?'}:${view.viewport?.width || '-'}x${view.viewport?.height || '-'}`).join(', ')}` : '-'],
    ['Panel material opacity', telemetry.materialDiagnostics ? `${telemetry.materialDiagnostics.transparentCount}/${telemetry.materialDiagnostics.total} transparent` : '-'],
    ['Panel material maps', telemetry.materialDiagnostics ? `${telemetry.materialDiagnostics.mappedCount}/${telemetry.materialDiagnostics.total}` : '-'],
    ['Panel diagnostic materials', telemetry.materialDiagnostics ? `${telemetry.materialDiagnostics.strictDiagnosticCount}: ${telemetry.materialDiagnostics.strictDiagnosticPanelIds?.join(', ') || '-'}` : '-'],
    ['Server summary', serverSummary?.version || serverSummaryError || '-'],
    ['Server surface', currentServerClient?.surface ? `${currentServerClient.surface.surfaceKind || '-'}:${currentServerClient.surface.entrypoint || '-'}` : '-'],
    ...
    ['Server base layer', currentServerSession?.renderState?.baseLayer ? (currentServerSession.renderState.baseLayer.present ? `${currentServerSession.renderState.baseLayer.framebufferWidth || '-'}x${currentServerSession.renderState.baseLayer.framebufferHeight || '-'}` : 'missing') : '-'],
    ['Server viewports', currentServerSession?.viewports ? `${currentServerSession.viewports.viewCount}: ${currentServerSession.viewports.views?.map((view) => `${view.eye || '?'}:${view.viewport?.width || '-'}x${view.viewport?.height || '-'}`).join(', ')}` : '-'],
    ['Server material opacity', currentServerSession?.materialDiagnostics ? `${currentServerSession.materialDiagnostics.transparentCount}/${currentServerSession.materialDiagnostics.total} transparent` : '-'],
    ['Server material maps', currentServerSession?.materialDiagnostics ? `${currentServerSession.materialDiagnostics.mappedCount}/${currentServerSession.materialDiagnostics.total}` : '-'],
    ['Server diagnostic materials', currentServerSession?.materialDiagnostics ? `${currentServerSession.materialDiagnostics.strictDiagnosticCount}: ${currentServerSession.materialDiagnostics.strictDiagnosticPanelIds?.join(', ') || '-'}` : '-'],
    ...
    ['Server Three HTMLTexture', currentServerHtmlCanvas?.threeTexture ? (currentServerHtmlCanvas.threeTexture.htmlTextureAvailable ? `available:${currentServerHtmlCanvas.threeTexture.threeRevision || 'unknown'}` : currentServerHtmlCanvas.threeTexture.reason || 'missing') : '-'],
    ['Server Three texture capability', currentServerHtmlCanvas?.threeTexture ? (currentServerHtmlCanvas.threeTexture.ready ? 'ready' : currentServerHtmlCanvas.threeTexture.reason || '-') : '-'],
