...
describe('portal orchestrator MCP tools', () => {
  ...
  it('allows required proof marker followed by a closing markdown fence before workflow result', async () => {
    let chat = sg.createChat({ name: 'Fenced proof marker result chat' }, 'test');
    sg.updateChatTask(chat.id, 'task-fenced-proof-marker');
    sg.set('tasks/task-fenced-proof-marker', {
      status: 'done',
      chatId: chat.id,
      prompt: 'Audit workflow kanban and return WORKFLOW_KANBAN_RELEASE_GATE:*.',
    }, 'test');
    ...
    sg.appendChatMessage(chat.id, {
      role: 'agent',
      text: response,
      taskId: 'task-fenced-proof-marker',
      streaming: false,
    });
    proxyManager.requestFromChild = async (serverName, method, params) => {
      internalCalls.push({ serverName, method, params });
    ...
    let result = await handlePortalOrchestratorTool(
      proxyManager,
      'get_chat_task_result',
      { chatId: chat.id, taskId: 'task-fenced-proof-marker' },
      'test',
      { stateGraph: sg },
    );
    let payload = JSON.parse(result.content[0].text);
  ...
  it('flags marker-only PASS final text as not closure-ready', async () => {
    let chat = sg.createChat({ name: 'Marker-only result chat' }, 'test');
    sg.updateChatTask(chat.id, 'task-marker-only');
    sg.set('tasks/task-marker-only', {
      status: 'done',
      chatId: chat.id,
    }, 'test');
    let response = [
    ...
    let result = await handlePortalOrchestratorTool(
      proxyManager,
      'get_chat_task_result',
      { chatId: chat.id, taskId: 'task-marker-only' },
      'test',
      { stateGraph: sg },
    );
    let payload = JSON.parse(result.content[0].text);
