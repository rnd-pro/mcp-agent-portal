...
describe('agent chat input state', () => {
  ...
  it('keeps a visible voice preview when recording cannot start or produce text', () => {
    ...
    assert.match(source, /Adapter\/resource defaults belong to the chat payload; the composer only keeps a compact settings entry point\./);
    assert.match(source, /this\.\$\.composerFooterControls = \[\{\s+id: 'settings'/);
